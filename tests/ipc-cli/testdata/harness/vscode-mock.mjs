		...
		commands: {
			...
			async executeCommand(command, ...args) {
				...
				if (command === 'vscode.openFolder') {
					state.openFolderCalled = true;
					const folderUri = args[0];
					state.openFolderPath = folderUri?.fsPath ?? folderUri;
					const options = args[1] ?? {};
					state.openFolderForceNewWindow = options.forceNewWindow === true;
				}
				if (command === 'git.openRepository') {
					state.gitOpenCalled = true;
					state.gitOpenPath = args[0];
				}
			},
		},
		window: {
